...
// CONFIGURACIÓN BACKTEST FINAL - ÚLTIMAS 24 HORAS
// Con Lógica de Cruce RSI (Sniper Real)
const APP_ID = 1089;
...
ws.on('open', () => {
    console.log("🥇 Iniciando Backtest Auditoría Final - ÚLTIMAS 24 HORAS...");
    ws.send(JSON.stringify({
        ticks_history: SYMBOL,
        end: 'latest',
        count: 500, // ~41 horas en M5
        style: 'candles',
        granularity: 300
    }));
});
...
function runBacktest(candles) {
    ...
    // Tomamos exactamente las últimas 24 horas (288 velas de 5 min)
    const startIndex = candles.length - 288;
    ...
    for (let i = EMA_PERIOD; i < candles.length; i++) {
        ...
        if (!activeTrade && isWithinRange) {
            // LÓGICA DE CRUCE RSI (Lo que acabamos de instalar)
    ...
    console.log("\n========================================");
    console.log("🥇 AUDITORÍA FINAL SNIPER (Últimas 24h)");
    console.log("========================================");
    console.log(`Periodo: ${new Date(candles[startIndex].epoch * 1000).toLocaleString('es-VE')} hasta ahora`);
    console.log(`Total Trades: ${history.length} disparos precisos`);
    console.log(`Ganados 🟢: ${wins} | Perdidos 🔴: ${losses}`);
    console.log("----------------------------------------");
    console.log(`PnL NETO (Stake $10): $${totalPnL.toFixed(2)} USD`);
    console.log(`PnL NETO (Stake $50): $${(totalPnL * 5).toFixed(2)} USD`);
    console.log(`Retorno diario (aprox): ${((totalPnL / 10) * 100).toFixed(1)}%`);
    console.log("========================================\n");

    if (history.length > 0) {
        console.log("Muestra de los últimos trades:");
        history.slice(-10).forEach((h, idx) => {
            console.log(`${idx + 1}. [${h.time.split(', ')[1]}] ${h.type} -> PnL: ${h.profit > 0 ? '🟢' : '🔴'} $${h.profit.toFixed(2)} (Stake $10)`);
        });
    }
}
