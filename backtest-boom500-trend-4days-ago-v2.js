...
// Fecha: Hace 4 días (07 de Marzo de 2026)
const targetDate = new Date('2026-03-07T00:00:00Z');
const endTS = Math.floor(new Date('2026-03-07T23:59:59Z').getTime() / 1000);
...
const CONFIG = {
    stake: 60,
    takeProfit: 1.00, // Optimización Final
    stopLoss: 2.50,   // Tu SL de seguridad
    multiplier: 200,
    timeStopTicks: 150,
    ...
    cooldownSecondsLoss: 60 // Recarga optimizada
};
...
ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para BOOM 500 (Fecha: 2026-03-07 - HACE 4 DÍAS)...`);
    fetchTicks(endTS);
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && prices.length > 0 && times[0] > (startTS - 5000)) {
            ...
            console.log(`\n✅ DATA CARGADA: ${allTicks.length} ticks. Analizando Estrategia Optimizada...`);
...
function runTrendGrinderBacktest() {
    ...
    for (let i = 4000; i < allTicks.length; i++) {
        ...
        if (inTrade) {
            ...
            else if (secondsElapsed >= CONFIG.timeStopTicks && profit < 1.00) { closed = true; reason = "⏱️ TIME-STOP"; }
    ...
    console.log("\n=========================================");
    console.log("🌾 RESULTADO 07 MARZO: BOOM 500");
    console.log("=========================================");
    console.log(`Total Trades Realizados: ${trades}`);
    console.log(`Ganadas: ${wins} ✅ | Perdidas: ${losses} ❌`);
    console.log(`PnL Neto de ese día: $${balance.toFixed(2)} 💰`);
    console.log(`Win Rate de ese día: ${((wins / (trades || 1)) * 100).toFixed(1)}%`);
    console.log("-----------------------------------------");
    const lossesList = tradeHistory.filter(t => t.profit < 0);
    if (lossesList.length === 0) console.log("Ese día fue perfecto. Cero spikes detectados.");
    else {
        console.log(`Spikes Incidentes: ${lossesList.length}`);
        lossesList.slice(-10).forEach(s => console.log(`[${s.time}] 🚨 ${s.reason} | PnL: ${s.profit.toFixed(2)}$`));
    }
    console.log("=========================================\n");
}
