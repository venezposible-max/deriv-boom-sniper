...
const endTS = Math.floor(new Date('2026-03-10T23:59:59Z').getTime() / 1000);
...
ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para BOOM 500 (Fecha: 2026-03-10 - AYER)...`);
    fetchTicks(endTS);
});
...
function runTrendGrinderBacktest() {
    ...
    console.log("\n=========================================");
    console.log("🌾 RESULTADO AYER (10 MARZO): BOOM 500");
    console.log("=========================================");
    ...
    console.log(`Win Rate de Ayer: ${((wins / (trades || 1)) * 100).toFixed(1)}%`);
    console.log("-----------------------------------------");
    const lossesList = tradeHistory.filter(t => t.profit < 0);
    if (lossesList.length === 0) console.log("Ayer fue un día perfecto. Cero spikes detectados.");
    else {
