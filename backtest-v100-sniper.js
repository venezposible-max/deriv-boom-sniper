...
// CONFIGURACIÓN BACKTEST V100 - ESTRATEGIA SNIPER
const APP_ID = 1089;
const SYMBOL = 'R_100'; // Volatility 100 Index
const STAKE = 10;
const MULTIPLIER = 200;
const TP_MOVE = 1.00; // El mismo $1 de movimiento del oro
const SL_MOVE = 0.50; // El mismo $0.50 de movimiento del oro
const RSI_PERIOD = 14;
const EMA_PERIOD = 20;
...
ws.on('open', () => {
    console.log("📊 Iniciando Backtest V100 con Estrategia Sniper (24 Horas)...");
...
function runBacktest(candles) {
    ...
    for (let i = EMA_PERIOD; i < candles.length; i++) {
        ...
        if (!activeTrade && isWithinRange) {
            // LÓGICA DE CRUCE RSI (La misma del oro)
    ...
    console.log("\n========================================");
    console.log("📊 RESULTADO SNIPER EN VOLATILITY 100");
    console.log("========================================");
    ...
    if (history.length > 0) {
        console.log("Detalle de los últimos trades en V100:");
        history.slice(-10).forEach((h, idx) => {
            console.log(`${idx + 1}. ${h.type} -> PnL: ${h.profit > 0 ? '🟢' : '🔴'} $${h.profit.toFixed(2)}`);
        });
    } else {
        console.log("No se generaron trades con esta configuración en V100.");
    }
}
