...
const APP_ID = 1089;
const TOTAL_TICKS_NEEDED = 55000; // ~24 Horas

const SPIKE_THRESHOLD = 0.5; // Definición de Spike (Salto mayor a 0.5 puntos)
...
ws.on('open', () => {
    console.log(`🔍 BUSCANDO SPIKES "EN EL PUNTO EXACTO" (RSI entre 70.0 y 71.0) - BOOM 500 (24H)...`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && prices.length > 0) {
            ...
            console.log(`\n✅ DATA CARGADA. Analizando el punto de entrada RSI 70.0...`);
            auditExactRSI70();
            ws.close();
        }
    }
...
function auditExactRSI70() {
    let spikesAtExactPoint = 0;
    let totalSpikes = 0;
    let details = [];
    ...
    for (let i = 4000; i < allTicks.length; i++) {
        ...
        if (diff >= SPIKE_THRESHOLD) {
            ...
            // Calculamos RSI justo ANTES del spike
            ...
            // "Punto Exacto" = RSI entre 69.5 y 70.5 (Donde el bot suele gatillar la entrada)
    ...
    console.log("\n=================================================");
    console.log("🕵️‍♂️ DETECTOR DE TRAMPAS: PUNTO EXACTO RSI 70");
    console.log("=================================================");
    console.log(`Total Spikes en 24h: ${totalSpikes}`);
    console.log(`Spikes ocurridos JUSTO al tocar RSI 70 (±0.5): ${spikesAtExactPoint} 🚩`);
    console.log(`Nivel de "Mala Suerte" al entrar: ${((spikesAtExactPoint / totalSpikes) * 100).toFixed(1)}%`);
    console.log("-------------------------------------------------");
    if (details.length > 0) {
        console.log("Momentos donde el RSI 70 fue una trampa:");
        details.forEach(s => {
            console.log(`[${s.time}] 🚨 Spike de ${s.size} pts | RSI exacto: ${s.rsi}`);
        });
    } else {
        console.log("✅ Limpio: Ningún spike ocurrió exactamente en el punto de RSI 70.");
    }
    console.log("=================================================\n");
}
