...
const TOTAL_TICKS_NEEDED = 350000; // ~8 Días de datos
...
ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para BOOM 500 (Semana Pasada: 01-08 Marzo)...`);
    fetchTicks(endTS);
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && prices.length > 0 && times[0] > startTS) {
            process.stdout.write('.');
            fetchTicks(times[0]);
        } else {
            console.log(`\n\n✅ DATA SEMANAL CARGADA: ${allTicks.length} ticks. Analizando Trend Grinder (RSI 75)...`);
...
function runTrendGrinderBacktest() {
    ...
    for (let i = 4000; i < allTicks.length; i++) {
        ...
        if (inTrade) {
            ...
            if (closed) {
                ...
                inTrade = false;
                tradeHistory.push({ time: new Date(openTime * 1000).toLocaleDateString() + ' ' + new Date(openTime * 1000).toLocaleTimeString(), profit, reason });
            }
    ...
    console.log("\n=========================================");
    console.log("🌾 TREND GRINDER: BOOM 500 (SEMANA PASADA)");
    console.log("=========================================");
    console.log(`Periodo: 01 de Marzo al 08 de Marzo, 2026`);
    console.log(`Configuración: RSI 75 | Cooldown 180s`);
    ...
    console.log(`Total Spikes Incidentes: ${spikeLosses.length}`);
    if (spikeLosses.length > 0) {
        console.log("Muestra de pérdidas:");
        spikeLosses.slice(-10).forEach(s => console.log(`[${s.time}] 🚨 ${s.reason} | PnL: ${s.profit.toFixed(2)}$`));
    } else {
        console.log("✅ ¡Semana perfecta! Ninguna sorpresa detectada.");
    }
    console.log("=========================================\n");
}
