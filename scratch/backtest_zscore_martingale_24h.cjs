...
function getAdjustedStake(baseStake, martingaleStep, debtQueue) {
    let adjusted = baseStake;
    
    if (debtQueue > 0) {
        // Nivel 2: Split Recovery (Tope de seguridad de $5.00 por cada $1.00 de baseStake)
        const targetPart = debtQueue / 4;
        const rawRecoveryStake = targetPart / 0.0909;
        const maxStakeCap = 5.0 * (baseStake / 1.0);
...
function runBacktestSimulation() {
    console.log('\n=============================================================');
    console.log('🧪 INICIANDO SIMULACIÓN DE BACKTEST CON COBERTURA (MARTINGALA)');
    console.log('=============================================================');
    console.log('Configuración:');
    console.log('- Stake Base: $1.00');
    console.log('- Periodo: Últimas 24 horas');
    console.log('- Cooldown: 30 segundos');
    console.log('- Nivel 1: Martingala x11');
    console.log('- Nivel 2: Split Recovery (Capped at $5.00, Max Debt $24.00)');
    console.log('=============================================================\n');
    ...
    const markets = {
        R_50: { digitHistory: [], lastTradeTime: 0, shannonEntropy: 3.322, lockedUntil: 0 },
        R_75: { digitHistory: [], lastTradeTime: 0, shannonEntropy: 3.322, lockedUntil: 0 }
    };
    
    // Bot parameters
    const baseStake = 1.00;
    let pnl = 0.0;
    let wins = 0;
    let losses = 0;
    let totalTrades = 0;
    ...
    for (let idx = 0; idx < combinedEvents.length; idx++) {
        ...
        let currentCooldown = 30000;
        const isRecovery = martingaleStep > 0 || debtQueue > 0;
        if (isRecovery) {
            currentCooldown = 60000; // 60s cooldown in recovery
        }
        
        if (now - lastGlobalTradeTime < currentCooldown) continue;
        
        // Locked symbol check (5m quarantine after a loss on that symbol)
        ...
        // Entropy filter (3.26 base, 3.24 recovery)
        const entropyLimit = isRecovery ? 3.24 : 3.26;
        if (entropyVal >= entropyLimit) continue;
        ...
        if (isRecovery) {
            zThreshold -= 0.3; // Stricter Z-score in recovery
        }
        ...
        if (bestTarget !== -1) {
            ...
            if (nextDigit === null) continue;
            
            const isWin = nextDigit !== bestTarget;
            const stake = getAdjustedStake(baseStake, martingaleStep, debtQueue);
            
            // Profit is ~9.09% of stake for DIGITDIFF (multiplied by 0.09)
            const profit = isWin ? parseFloat((stake * 0.0909).toFixed(2)) : -stake;
            ...
            const lastMartingaleStep = martingaleStep;
            const lastDebtQueue = debtQueue;
            const recoveryStateString = debtQueue > 0 
                ? `Nivel 2 (Deuda: $${debtQueue.toFixed(2)})` 
                : (martingaleStep > 0 ? `Nivel 1 (x11)` : 'Base');
            
            if (isWin) {
                wins++;
                // Handle recovery logic on win
                if (debtQueue > 0) {
                    debtQueue -= profit;
                    if (debtQueue <= 0.01) {
                        debtQueue = 0;
                ...
                losses++;
                // Quarantine loser symbol for 5 minutes
                mState.lockedUntil = now + 300000;
                
                // Handle recovery logic on loss
                if (debtQueue > 0) {
                    debtQueue += Math.abs(profit);
                    
                    // Cap check
                    const absoluteDebtCap = 24.0 * baseStake;
                    if (debtQueue > absoluteDebtCap) {
                        debtQueue = 0;
                        martingaleStep = 0;
                    }
                } else if (martingaleStep === 1) {
                    debtQueue = Math.abs(profit) + baseStake; // $11.00 + $1.00 = $12.00
                    martingaleStep = 0;
                } else {
                    martingaleStep = 1;
                }
            ...
            tradeLog.push({
                ...
                recoveryState: recoveryStateString
            });
        }
    }
    ...
    // Output Report
    console.log('\n' + '═'.repeat(75));
    console.log('📊 REPORTE DE RENDIMIENTO CON RECOV/MARTINGALA (Últimas 24h Reales)');
    ...
    if (tradeLog.length > 0) {
        console.log(`\n📋 BITÁCORA DE ÚLTIMOS 25 DISPAROS (MUESTRA DE COMPORTAMIENTO):`);
        console.log(`   ┌──────────────────────┬──────┬────────┬───────┬──────┬─────────┬──────────┬───────────┬───────────────────────────┐`);
        console.log(`   │ Hora                 │ Act. │ Z-sc   │ Barr  │ Out  │ Stake   │ Res      │ PnL Acum  │ Estado Cobertura          │`);
        console.log(`   ├──────────────────────┼──────┼────────┼───────┼──────┼─────────┼──────────┼───────────┼───────────────────────────┤`);
        tradeLog.slice(-25).forEach(t => {
            const resSymbol = t.result === 'WIN ✅' ? ' WIN ✅  ' : 'LOSS ❌  ';
            console.log(`   │ ${t.time.padEnd(20)} │ ${t.symbol.padEnd(4)} │ ${t.z.padStart(6)} │  NO-${t.barrier} │  ${t.outcome}   │ $${t.stake.padStart(5)} │${resSymbol}│ $${t.totalPnl.padStart(8)} │ ${t.recoveryState.padEnd(25)} │`);
        });
        console.log(`   └──────────────────────┴──────┴────────┴───────┴──────┴─────────┴──────────┴───────────┴───────────────────────────┘`);
    }
}
